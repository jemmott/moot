...
def pbbTool():
    def backupToPBB(ipAddress, fileName):
        ...
        PBB.fromIpAddress(ipAddress, verbose=args.verbose).toFile(
            fileName, args.explode
        )
    ...
    def cloneFromPBB(ipAddress, fileName):
        ...
        for filename in pb.getFileList(
            PBB.fileTypes.filePattern | PBB.fileTypes.filePatternSetting
        ):
            if args.verbose:
    ...
    def extractFromPBB(pbbFile, patternName):
        ...
        for fileName in pbb.getFileList(PBB.fileTypes.filePattern):
            ...
            if fnmatch.fnmatch(pbp.name, patternName):
                ...
                print(f"  {pbp.name}")
                pbp.toEPE().toFile(
                    pathlib.Path(pbbFile).with_name(pbp.name).with_suffix(".epe")
                )
    ...
    parserBackup = subparsers.add_parser(
        "backup", help="backup --ipAddress=* --pbbFile=*"
    )
    parserBackup.add_argument(
        "--ipAddress",
        default="*",
        help="The (wildcard-enabled) IP address of the Pixelblaze to backup FROM; defaults to '*'",
    )
    parserBackup.add_argument(
        "--pbbFile",
        default="*",
        help="The (wildcard-enabled) filename of the PixelBlazeBackup (PBB) file to backup TO; defaults to './*'",
    )
    parserBackup.add_argument(
        "--explode",
        action="store_true",
        help="Explode the backup file into its components",
    )
    parserBackup.add_argument(
        "--verbose", action="store_true", help="Display debugging output"
    )
    ...
    parserRestore = subparsers.add_parser(
        "restore", help="restore --ipAddress={ipAddress} --pbbFile={pbbFile.pbb}"
    )
    parserRestore.add_argument(
        "--pbbFile",
        required=True,
        help="The filename of the PixelBlazeBackup (PBB) file to restore FROM",
    )
    parserRestore.add_argument(
        "--ipAddress",
        required=True,
        help="The IP address of the Pixelblaze to restore TO",
    )
    parserRestore.add_argument(
        "--verbose", action="store_true", help="Display debugging output"
    )
    ...
    parserClone = subparsers.add_parser(
        "clone", help="clone --fromAddress={ipAddress} --toAddress={ipAddress}"
    )
    parserClone.add_argument(
        "--pbbFile",
        required=True,
        help="The filename of the PixelBlazeBackup (PBB) file to clone FROM",
    )
    parserClone.add_argument(
        "--ipAddress",
        required=True,
        help="The IP address of the Pixelblaze to clone TO",
    )
    parserClone.add_argument(
        "--verbose", action="store_true", help="Display debugging output"
    )
    ...
    listGroup.add_argument(
        "--pbbFile", help="The filename of the PixelBlazeBackup (PBB) file to list"
    )
    listGroup.add_argument(
        "--ipAddress",
        default="*",
        help="The (wildcard-enabled) IP address of the Pixelblaze to list",
    )
    ...
    parserExtract = subparsers.add_parser(
        "extract", help="extract --pbbFile={pbbFile.pbb} --patternName=*"
    )
    parserExtract.add_argument(
        "--pbbFile",
        required=True,
        help="The filename of the PixelBlazeBackup (PBB) file to extract",
    )
    parserExtract.add_argument(
        "--patternName",
        required=True,
        default="*",
        help="The (wildcard-enabled) name of the pattern(s) to extract",
    )
    parserExtract.add_argument(
        "--verbose", action="store_true", help="Display debugging output"
    )
            ...
            if fnmatch.fnmatch(ipAddress, args.ipAddress):
                with Pixelblaze(ipAddress) as pixelblaze:
                    ...
                    fileName = pathlib.Path(
                        args.pbbFile.replace("*", pixelblaze.getDeviceName())
                    ).with_suffix(".pbb")
